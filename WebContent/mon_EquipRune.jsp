...
String item_uniqueID= request.getParameter("item_uniqueID");//장착 아이템 고유번호
String rune_pos		= request.getParameter("rune_pos");		//장착위치
...
// 장착 및 장착해제 	
	...
	cs = Conn.prepareCall("{call abn_mgmtEquipRune(?,?,?,?,?,?,?,?,?)}");
	cs.setString(1,job_type);
	cs.setString(2,user_account);
	cs.setString(3,mon_id);
	cs.setString(4,user_mon_sn);
	cs.setString(5,item_id);
	cs.setString(6,item_uniqueID);
	cs.setString(7,rune_pos);
		
	cs.registerOutParameter(8, java.sql.Types.INTEGER);	
	cs.registerOutParameter(9, java.sql.Types.VARCHAR);
	cs.execute();
	result_cd 	= cs.getInt(8);
	result_msg 	= cs.getString(9);
...
<%=P_type %>|<%=result_cd %>|<%=job_type%>|<%=tot_cnt%>|<%=result_packet%>