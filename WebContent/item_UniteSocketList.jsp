...
<%
String P_type ="T5931";
String tre_type = request.getParameter("tre_type");
...
try{
	...
	sql.append(" Call abn_getSocketTreasureList  (5," + user_account+",null,null,null,null)\n") ;
...
//즉시합성비용 
	sql.setLength(0);
	sql.append(" select var_value from MGR_VARIABLE where var_id='TRE_UNITED_AMT' \n") ;
  	pstmt = Conn.prepareStatement(sql.toString());
  	
  	rs = pstmt.executeQuery();
  	
  	if(rs.next()) {
  		unite_amt = rs.getInt("var_value");
  	}
...
<%=P_type %>|<%=unite_amt%>|<%=tot_cnt %>|<%for(int i=0;i<list.size();i++){%>
