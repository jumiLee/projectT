...
try{
	//System.out.println ("P_type :" + P_type  + ", item_type:" + item_type);
...
// 아이템정보
	if (job_type.equals("1")) { //All
		...
		sql.append(" CALL abn_getItemList (6,"+user_account+ ","+ item_type +") \n");
	}

	//System.out.println ("abn_getItemList:" + sql.toString());
...
//System.out.println ("result:" + result_cd);	
	
	
	while(rs.next()){
		map = new HashMap();
		/*	total_cnt|
			item_id|item_uniqueID|item_type|item_nm|item_img_no|item_rare_degree|item_usage|item_cnt|item_desc|item_price|
			item_value|ATT|DEF|HP|SP|abnormal|speedup_all|speedup|exp_up|critical_up|
			reinforce_up|limit_lv|item_period_flag|item_period|remain_time(s)|item_class|item_grade|item_level|item_exp|item_rfc_flag|
			item_rfc_mtr_flag|	*/
		...
		map.put("check_flag"		, rs.getInt("check_flag"));			//added 2015-02-04
		map.put("item_upgrade_cost"	, rs.getInt("item_upgrade_cost"));	//added 2015-04-28
...
//Save item socket master info Map 	
	...
	//System.out.println("map:"+map_my_sok.get("item_uniqueID"));
...
//몬스터 장착정보 List
	...
	for(int eq=0;eq<list.size();eq++){
		...
		result_packet += 	list.get(eq).get("item_id").toString() 		+"|"+
							...
							list.get(eq).get("check_flag").toString()			+"|"+
							list.get(eq).get("item_upgrade_cost").toString()	+"|";
	...
	//몬스터 장착아이템 소켓정보 List 개수	
		...
		for(int my_socket=0;my_socket<list_my_sok.size();my_socket++){
			// System.out.println ("list_my_sok.size(1):" + list_my_sok.size());
