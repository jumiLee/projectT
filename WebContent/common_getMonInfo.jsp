...
//랜덤 대전상대 관련 	
	ResultSet rs_rand = null;
	PreparedStatement pstmt_rand = null;	
...
try{
	...
	//랜덤 대전상대 뽑기 
	if(bat_user_account.equals("-1")) {	// 랜덤으로 상대를 뽑아야 하는경우 bat_user_account 를 -1로 받음
		
		sql.append(" Call abn_getFriendList (9, " + user_account + ", null) \n");
		pstmt_rand = Conn.prepareStatement(sql.toString());
		rs_rand = pstmt_rand.executeQuery();	
	
		if (rs_rand.next()){
			bat_user_account = rs_rand.getString("user_account");
		}
//System.out.println ("Rand PVP account :" + bat_user_account);		
	}	
	...
	if(pstmt_my_sok != null) pstmt_eq.close();
	if(rs_my_sok != null) rs_eq.close();
	if(pstmt_rand != null) pstmt_eq.close();
	if(rs_rand != null) rs_eq.close();
